...
public class SuspendVendorCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_WhenVendorIsNotActive_ThrowsBusinessRuleException()
    {
        var vendor = new Vendor(Guid.NewGuid(), "Ar", "En", "Retail", "CR", "t@t.com", "1");
        ...
        var exception = await Assert.ThrowsAsync<BusinessRuleException>(() =>
            handler.Handle(new SuspendVendorCommand(vendor.Id, "Policy violation"), default));

        exception.Message.Should().Contain("cannot be suspended");
